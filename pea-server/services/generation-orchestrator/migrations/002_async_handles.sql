...
-- 注意: ai_providers 真实列序为 id,name,provider_type,base_url,api_key,...
--       故新列追加在 api_key 之后 (提案草稿写的 model_name 在真实表不存在, 已修正)。

-- 1) ai_providers 增加完成模式契约列
ALTER TABLE ai_providers
  ADD COLUMN completion_mode VARCHAR(16) NOT NULL DEFAULT 'poll' AFTER api_key,
...
--    user_id 用 BIGINT 与 generation_jobs.user_id(bigint unsigned) 对齐。
