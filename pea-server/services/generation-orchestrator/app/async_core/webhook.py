...
def sign_webhook(job_id: str, provider_task_id: str, secret: str) -> str:
    msg = f"{job_id}:{provider_task_id}".encode("utf-8")
    return hmac.new(secret.encode("utf-8"), msg, hashlib.sha256).hexdigest()


def verify(job_id: str, provider_task_id: str, sig: str, secret: str) -> bool:
    if not secret or not sig:
        return False
    return hmac.compare_digest(sign_webhook(job_id, provider_task_id, secret), sig)
...
async def webhook(
    ...
    if not verify(job_id, provider_task_id, sig, secret):
    ...
    user_id = int(handle.get("user_id", 0) or 0)
