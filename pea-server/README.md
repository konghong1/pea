...
| E5 画布编辑器 M1 | 节点/连线/Inspector/Agent/生成 | 🟡 基础已通（ReactFlow+自动保存+生成接入）；Agent 对话/富文本(⏳ 后续) |
| E6 电商套图 M2 | — | ⏸ 搁置（决策不变） |
| E7 全局系统 G | 顶栏积分/Toast/通知/分享 | 🟡 顶栏积分+WS 通知已通；Toast/分享(⏳) |
| E8/E9 主页/社区 | — | ⏳ P1 未排期 |
| E10 质量门 | 单测/E2E/性能/发布 | ⏳ 框架待补（见 §6） |

> 说明：以上为**可运行脚手架 + 核心两线打通**。标记为 ⏳ 的是后续迭代项，接入真实大模型密钥后 Worker 写产出与 LiteLLM 路由即可启用，无需重构。
