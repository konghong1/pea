"""精细化复现：刷新后观察 html class / body surface 的时序变化，并抓取 console。"""
...
def main():
    with sync_playwright() as p:
        ...
        ts = int(time.time())
        page.fill('input[placeholder="you@pea.ai"]', f"tt2_{ts}@pea.dev")
        page.fill('input[placeholder="至少 8 位"]', "Password123")
        page.fill('input[placeholder="可选"]', "TT2")
        ...
        print("\n=== CONSOLE LOGS ===")
        for line in logs:
            if "theme" in line.lower() or "workspace" in line.lower() or "creator" in line.lower():
                print(line)

        page.screenshot(path=str(SHOTS / "theme_refresh_timing2.png"), full_page=False)
        browser.close()
